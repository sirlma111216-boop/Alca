...
export declare const LIVE_PROTOCOL_VERSION = "1.0";
/** 수업 코드 길이. */
export declare const CODE_LENGTH = 6;
/** 한 방에 들어올 수 있는 최대 인원. core 의 MAX_PARTICIPANTS 와 맞춘다. */
export declare const ROOM_MAX_MEMBERS = 40;
/** 닉네임 길이 상한. */
export declare const NICK_MAX = 12;
...
export declare const encode: (msg: Envelope) => string;
export declare function decode(raw: string | ArrayBuffer): Envelope | null;
export declare const ack: (id: number | undefined, data: unknown) => string;
export declare const nack: (id: number | undefined, message: string) => string;
export declare const push: (type: string, data: unknown) => string;
export declare function makeCode(): string;
/** 사람이 친 코드를 서버·브라우저 양쪽에서 **같은 방식으로** 다듬는다. */
export declare function normalizeCode(raw: unknown): string;
/**
 * 닉네임을 다듬는다. 정규식 대신 한 글자씩 걸러 escape 사고를 피한다.
 * 클라이언트만 믿지 않는다 — 서버에서 한 번 더 자른다.
 */
export declare function cleanNick(v: unknown): string;
/** 토큰은 기기가 만든 무작위 문자열이다. 길이만 자른다. */
export declare const cleanToken: (v: unknown) => string;
/** 방의 진행 단계. */
 ...
 */
export declare const DEMO_PREFIX = "demo-";
